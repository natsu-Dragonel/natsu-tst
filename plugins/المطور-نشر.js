...
const handler = async (m, { conn, text, groupMetadata }) => {
  ...
  // Get all chats (groups and private)
  const allChats = Object.entries(conn.chats).filter(([jid, chat]) => chat.isChats).map((v) => v[0]);

  const fakegif = { key: { participant: `0@s.whatsapp.net`, ...('6289643739077-1613049930@g.us' ? { remoteJid: '6289643739077-1613049930@g.us' } : {}) }, message: { 'videoMessage': { 'title': '𝑁𝐴𝑇𝑆𝑈Bᴏᴛ - MD🍁⃨፝⃕✰', 'h': `Hmm`, 'seconds': '99999', 'gifPlayback': 'true', 'caption': '💵 𝑁𝐴𝑇𝑆𝑈 𝐁𝐎𝐓 👑', 'jpegThumbnail': false } } };
